...
/*
Returns information about the petlink 32 bit listmode binary file. 
*/
...
Returns information about the petlink 32 bit listmode binary file. 
*/
extern int petlink32_to_static_sinogram(char *filename, int *n_packets, int *n_radial_bins, int *n_angles, int *n_sinograms, int *sinogram, int *n_prompts, int *n_delayed, int *n_tags, int *n_time, int *n_motion, int *n_monitoring, int *n_control) 
{
    int status = STATUS_SUCCESS; 
    ...
    int angle_index;
    int radial_index;
    int radial_bins = *n_radial_bins; 
    int angles = *n_angles; 
    int sinograms = *n_sinograms; 
    int N = radial_bins*angles; //just precopute this product 
    Packet packet; 
    for (i = 0; i < *n_packets; i++) {
        clear_packet(&packet);
        status = read_and_decode_packet(fid, &packet); 
        ...
        *n_control = *n_control + packet.is_control;
        ...
        if (packet.is_prompt) { //FIXME: also make a sinogram for the delayed events 
            bin_index = packet.bin_index; 
            sinogram_index = floor(bin_index / N); 
            angle_index    = floor((bin_index-sinogram_index*N)/radial_bins ); 
            radial_index   = bin_index-sinogram_index*N-angle_index*radial_bins; 
            // make sure that they don't exceed the size of the sinogram: 
            // .. FIXME 
            sinogram[radial_index*angles*sinograms+angle_index*sinograms+sinogram_index] = sinogram[radial_index*angles*sinograms+angle_index*sinograms+sinogram_index]+1; 
        }
    }
